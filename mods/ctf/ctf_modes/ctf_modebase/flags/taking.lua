		...
		for _, flagteam in pairs(flagteams) do
			...
			if capture then
				...
				if fpos then
					...
					ctf_core.error("ctf_modebase:flag_taking", "Failed to return flag to its base! Node at flag pos: "..dump(minetest.get_node(ctf_map.current_map.teams[flagteam].flag_pos)))
				end
			end
		end
